...
$('#claim_organization_list').live('change', function(event, ui){
   ...
   dep_link = '/kurator.php/claimopened/departmentslist/orgid/'+$(this).val();
   city_link = '/kurator.php/ajaxdata/city/orgid/'+$(this).val();
   client_link = '/kurator.php/ajaxdata/clients/orgid/'+$(this).val();
   importance_link = '/kurator.php/ajaxdata/importance/orgid/'+$(this).val();
   //dep_list.html('');
...
$('#claim_city_list').live('change', function(event, ui){
   ...
   dep_list.before('<div class="loading_image claim_new" id="claim_departments_id_loading">Загрузка</div>');
   dep_link = '/kurator.php/ajaxdata/departments_by_org/cityid/'+$(this).val()+'/orgid/'+organization.val();
   dep_list.load(dep_link, function(){
     $('#claim_departments_id_loading').remove();
   });
...
function auto_organization_once()
{
  ...
  var org_id = organizationHolder.val();

  var urlClients = organizationHolder.data('url_clients');
  var urlImportance = organizationHolder.data('url_importance');
  ...
  client_link = urlClients + '/orgid/'+org_id;
  importance_link = urlImportance + '/orgid/'+org_id;
  //dep_list.html('');
