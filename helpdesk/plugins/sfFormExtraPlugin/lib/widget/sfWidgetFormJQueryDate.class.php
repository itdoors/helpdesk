...
class sfWidgetFormJQueryDate extends sfWidgetForm
{
  ...
  protected function configure($options = array(), $attributes = array())
  {
    ...
    $this->addOption('minMonth', '1');
    $this->addOption('maxMonth', '12');
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function() {
    jQuery("#%s").datepicker(jQuery.extend({}, {
      minDate:    new Date(%s, %s - 1, 1),
      maxDate:    new Date(%s, %s - 1, 31),
...
EOF
      ...
      $this->generateId($name.'[day]'), $this->generateId($name.'[day]'),
      $id,
      min($years), $this->getOption('minMonth'), max($years), $this->getOption('maxMonth'),
